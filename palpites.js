...
/* ============================================================
   MONTAR AUTOMATICAMENTE A SEGUNDA FASE (OPÇÃO B)
   ============================================================ */

// 1) Monta listas de 1º, 2º e 3º colocados
function montarListasClassificacao(classificacaoPorGrupo) {
  const primeiros = {};
  const segundos = {};
  const terceiros = [];

  for (const grupo of Object.keys(classificacaoPorGrupo)) {
    const tabela = classificacaoPorGrupo[grupo];

    primeiros[grupo] = { ...tabela[0], grupo };
    segundos[grupo] = { ...tabela[1], grupo };

    // O 3º vai concorrer às vagas extras
    terceiros.push({ ...tabela[2], grupo });
  }

  // Ordena os terceiros por pontos, saldo e gols pró
  terceiros.sort((a, b) => {
    if (b.pontos !== a.pontos) return b.pontos - a.pontos;

    const saldoA = (a.gp ?? 0) - (a.gc ?? 0);
    const saldoB = (b.gp ?? 0) - (b.gc ?? 0);
    if (saldoB !== saldoA) return saldoB - saldoA;

    return (b.gp ?? 0) - (a.gp ?? 0);
  });

  const melhoresTerceiros = terceiros.slice(0, 8); // TOP 8

  return { primeiros, segundos, melhoresTerceiros };
}

// 2) Função para alterar o nome no HTML
function setSlotTexto(id, time) {
  const el = document.getElementById(id);
  if (!el) return;

  el.textContent = time ? time.nome : "—";
}

// 3) Escolher terceiro apenas de grupos permitidos e sem repetir
function pegarTerceiroElegivel(lista, gruposPermitidos, usados) {
  for (const t of lista) {
    if (gruposPermitidos.includes(t.grupo) && !usados.has(t.grupo)) {
      usados.add(t.grupo);
      return t;
    }
  }
...
// 4) Preencher automaticamente a Segunda Fase inteira
function preencherSegundaFase(classificacaoPorGrupo) {
  const { primeiros, segundos, melhoresTerceiros } =
    montarListasClassificacao(classificacaoPorGrupo);

  const usados = new Set();

  // Jogos 1–8 (lado esquerdo)
  setSlotTexto("sf1_time1", primeiros.A);
  setSlotTexto("sf1_time2", pegarTerceiroElegivel(melhoresTerceiros, ["B", "C", "D"], usados));

  setSlotTexto("sf2_time1", primeiros.B);
  setSlotTexto("sf2_time2", pegarTerceiroElegivel(melhoresTerceiros, ["A", "E", "F"], usados));

  setSlotTexto("sf3_time1", primeiros.C);
  setSlotTexto("sf3_time2", segundos.D);

  setSlotTexto("sf4_time1", primeiros.D);
  setSlotTexto("sf4_time2", segundos.C);

  setSlotTexto("sf5_time1", primeiros.E);
  setSlotTexto("sf5_time2", segundos.F);

  setSlotTexto("sf6_time1", primeiros.F);
  setSlotTexto("sf6_time2", segundos.E);

  setSlotTexto("sf7_time1", primeiros.G);
  setSlotTexto("sf7_time2", segundos.H);

  setSlotTexto("sf8_time1", primeiros.H);
  setSlotTexto("sf8_time2", segundos.G);

  // Jogos 9–16 (lado direito)
  setSlotTexto("sf9_time1", primeiros.I);
  setSlotTexto("sf9_time2", pegarTerceiroElegivel(melhoresTerceiros, ["J", "K", "L"], usados));

  setSlotTexto("sf10_time1", primeiros.J);
  setSlotTexto("sf10_time2", pegarTerceiroElegivel(melhoresTerceiros, ["I", "K", "L"], usados));

  setSlotTexto("sf11_time1", primeiros.K);
  setSlotTexto("sf11_time2", segundos.L);

  setSlotTexto("sf12_time1", primeiros.L);
  setSlotTexto("sf12_time2", segundos.K);

  setSlotTexto("sf13_time1", segundos.A);
  setSlotTexto("sf13_time2", segundos.B);

  setSlotTexto("sf14_time1", segundos.C);
  setSlotTexto("sf14_time2", segundos.D);

  setSlotTexto("sf15_time1", segundos.E);
  setSlotTexto("sf15_time2", segundos.F);

  setSlotTexto("sf16_time1", segundos.G);
  setSlotTexto("sf16_time2", segundos.H);
}

preencherSegundaFase(classificacaoPorGrupo);
